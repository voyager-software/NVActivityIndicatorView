...
final class NVActivityIndicatorAnimationLineSpinFadeLoader: NVActivityIndicatorAnimationDelegate {
    func setUpAnimation(in layer: CALayer, size: CGSize, color: UIColor, lineWidth: CGFloat?) {
        ...
        // Draw lines
        for i in 0 ..< 8 {
            let line = self.lineAt(angle: CGFloat(Double.pi / 4 * Double(i)),
                                   size: lineSize,
    ...
    func lineAt(angle: CGFloat, size: CGSize, origin: CGPoint, containerSize: CGSize, color: UIColor, lineWidth: CGFloat?) -> CALayer {
        ...
        let line = NVActivityIndicatorShape.line.layerWith(size: size, color: color, lineWidth: lineWidth)
